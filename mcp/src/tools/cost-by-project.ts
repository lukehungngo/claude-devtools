...
export const costByProjectTool: ToolDefinition = {
  name: "cost_by_project",
  description: "Cost breakdown by project (sorted descending by cost).",
  inputSchema: {
    type: "object",
    properties: {
  ...
  handler: async (raw) => {
    ...
    const projects = new Map<string, { cost: number; tokens: number; sessionCount: number }>();
    ...
    for (const s of sessions) {
      ...
      const entry = projects.get(s.projectHash) ?? { cost: 0, tokens: 0, sessionCount: 0 };
      entry.cost += cost;
      entry.tokens += tokens;
      entry.sessionCount += 1;
      projects.set(s.projectHash, entry);
    }
...
registerTool(costByProjectTool);
