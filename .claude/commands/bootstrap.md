...
| `cd server && npx tsc --noEmit && cd ../dashboard && npx tsc --noEmit` | `mvn compile -DskipTests` (Java compiler IS the typechecker) | `gradle compileJava` |
| `make build` | `mvn package -DskipTests` | `gradle build -x test` |
...
| `cd server && npx tsc --noEmit && cd ../dashboard && npx tsc --noEmit` | `./mvnw compile -DskipTests` | `./gradlew compileJava` |
| `make build` | `./mvnw package -DskipTests` | `./gradlew build -x test` |
...
- `cd server && npx tsc --noEmit && cd ../dashboard && npx tsc --noEmit` → detected typecheck command
- `make build` → detected build command
- `TypeScript 5.x, React 18, Vite 5, TailwindCSS 3, Express 4, Vitest` → detected language + key libraries
- `true` for `has_ui` → detected from framework
