...
/**
 * Minimal shape required to derive context window from event stream.
 * Matches the loose event shape accepted by computeLiveMetrics in cost.ts
 * — dashboard code passes raw event-like objects from the WS/SSE stream,
 * not strongly-typed SessionEvent instances.
 */
...
export function deriveObservedContextWindow(
  events: readonly ContextWindowEvent[],
): number {
  let max = 0;
