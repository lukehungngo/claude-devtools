...
describe("MetricsCache", () => {
    ...
    it("invalidates when size changes", () => {
        ...
        const events = [];
        const subagentMeta = new Map();
        cache.set({ filePath: "/tmp/a.jsonl", size: 1000, mtimeMs: 12345 }, { metrics, events, subagentMeta });
    ...
    it("evicts oldest entry when max entries exceeded (LRU)", () => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            cache.set({ filePath: `/tmp/${i}.jsonl`, size: 100, mtimeMs: 100 }, { metrics: makeMetrics(), events, subagentMeta });
        }
        ...
        vi.advanceTimersByTime(100);
        cache.set({ filePath: "/tmp/5.jsonl", size: 100, mtimeMs: 100 }, { metrics: makeMetrics(), events, subagentMeta });
        // Entry 1 should be evicted (it was the LRU since 0 was accessed)
        expect(cache.get({ filePath: "/tmp/1.jsonl", size: 100, mtimeMs: 100 })).toBeNull();
        // Entry 0 should still be present
        expect(cache.get({ filePath: "/tmp/0.jsonl", size: 100, mtimeMs: 100 })).not.toBeNull();
        // Entry 5 should be present
        expect(cache.get({ filePath: "/tmp/5.jsonl", size: 100, mtimeMs: 100 })).not.toBeNull();
        vi.useRealTimers();
    });
    it("expires entries after TTL", () => {
        ...
        const events = [];
        const subagentMeta = new Map();
        shortCache.set({ filePath: "/tmp/a.jsonl", size: 100, mtimeMs: 100 }, { metrics: makeMetrics(), events, subagentMeta });
        // Before TTL
        expect(shortCache.get({ filePath: "/tmp/a.jsonl", size: 100, mtimeMs: 100 })).not.toBeNull();
        // After TTL
        vi.advanceTimersByTime(1500);
        expect(shortCache.get({ filePath: "/tmp/a.jsonl", size: 100, mtimeMs: 100 })).toBeNull();
        vi.useRealTimers();
    });
    it("reports cache size correctly", () => {
        ...
        cache.set({ filePath: "/tmp/a.jsonl", size: 100, mtimeMs: 100 }, { metrics: makeMetrics(), events, subagentMeta });
    ...
    it("clear() removes all entries", () => {
        ...
        const subagentMeta = new Map();
        cache.set({ filePath: "/tmp/a.jsonl", size: 100, mtimeMs: 100 }, { metrics: makeMetrics(), events, subagentMeta });
        cache.set({ filePath: "/tmp/b.jsonl", size: 200, mtimeMs: 200 }, { metrics: makeMetrics(), events, subagentMeta });
        cache.clear();
        expect(cache.size).toBe(0);
    });
});
//# sourceMappingURL=metrics-cache.test.js.map