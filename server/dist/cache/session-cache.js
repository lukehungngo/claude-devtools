...
export class SessionCache {
     ...
     */
    getSessionInfo(filePath, projectHash) {
    ...
    extractSessionInfo(filePath, projectHash, stat) {
        ...
        if (!sessionName && slug)
            sessionName = slug;
        // Count subagents
        const subagentDir = join(filePath.replace(`/${sessionId}.jsonl`, ""), sessionId, "subagents");
        let subagentCount = 0;
     ...
     */
    readHeadLines(filePath, fileSize) {
        if (fileSize === 0)
            return [];
     ...
     */
    readTailLines(filePath, fileSize) {
        if (fileSize === 0)
            return [];
