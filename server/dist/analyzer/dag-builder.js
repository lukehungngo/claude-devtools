            ...
            for (const content of normalizeContent(event.message.content)) {
                if (content.type === "tool_use") {
                    ...
                    if (content.name === "Agent" || content.name === "Task") {
                        const desc = content.input.description;
                        if (desc)
                            agentDescriptions.push(desc);
                    }
                }
            }
        }
...
export function buildAgentDAG(mainEvents, subagentEvents, subagentMeta) {
    const nodes = [];
    const edges = [];
    // Build descriptionToAgentId map for O(1) edge lookups
    const descriptionToAgentId = new Map();
 ...
 */
export function aggregateTokens(events) {
