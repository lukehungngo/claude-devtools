...
describe("computeMetrics", () => {
    it("aggregates tokens from main events only (subagent tokens tracked in DAG)", () => {
        ...
        const metrics = computeMetrics(makeSessionInfo(), mainEvents, subagentEvents, subagentMeta);
    ...
    it("calculates totalAgents as 1 + subagentEvents.size", () => {
        const mainEvents = [makeAssistantEvent()];
        const subagentEvents = new Map([
    ...
    it("computes contextPercent based on last assistant event input tokens", () => {
        ...
        const metrics = computeMetrics(makeSessionInfo(), mainEvents, new Map(), new Map());
    ...
    it("computes duration from first to last main event timestamps (inactive session)", () => {
        ...
        const metrics = computeMetrics(makeSessionInfo({ isActive: false }), mainEvents, new Map(), new Map());
        // 5 minutes = 300000 ms
    ...
    it("computes cumulative cost in tokensByTurn", () => {
        ...
        expect(metrics.tokensByTurn[1].cumulativeCost).toBeGreaterThan(metrics.tokensByTurn[0].cumulativeCost);
        expect(metrics.tokensByTurn[1].cumulativeCost).toBeCloseTo(metrics.tokensByTurn[0].cost + metrics.tokensByTurn[1].cost);
    });
