...
export function calculateTokenCost(model, tokens) {
    const pricing = Object.entries(MODEL_PRICING).find(([key]) => model.includes(key.split("-").slice(0, -1).join("-")) || model.includes(key))?.[1] || MODEL_PRICING["claude-sonnet-4-6"];
    return ((tokens.inputTokens * pricing.input) / 1_000_000 +
...
export function computeMetrics(sessionInfo, mainEvents, subagentEvents, subagentMeta) {
    const allEvents = [
        ...mainEvents,
