...
export function addSessionAllowance(sessionId, toolName) {
...
export function isToolAllowedForSession(sessionId, toolName) {
    return sessionAllowances.get(sessionId)?.has(toolName) ?? false;
}
export function getSessionAllowances(sessionId) {
    const tools = sessionAllowances.get(sessionId);
    return tools ? Array.from(tools) : [];
}
export function clearSessionAllowances() {
...
export function addPermissionRequest(data) {
    ...
    permissionLog.info({ id: permission.id, toolName: data.toolName, sessionId: data.sessionId, autoApproved }, autoApproved ? "permission auto-approved (session allowance)" : "permission requested");
    return permission;
}
...
export function cleanupPermissions() {
    ...
    if (resolved.length > 50) {
        const toRemove = resolved
            .sort((a, b) => new Date(a[1].timestamp).getTime() -
