...
export function discoverRepoGroups() {
...
export function loadFullSession(sessionInfo) {
    const mainEvents = parseJsonlFile(sessionInfo.path);
    const subagentEvents = new Map();
    const subagentMeta = new Map();
    const subagentDir = join(getClaudeProjectsDir(), sessionInfo.projectHash, sessionInfo.id, "subagents");
    if (existsSync(subagentDir)) {
        for (const file of readdirSync(subagentDir)) {
            if (file.endsWith(".jsonl")) {
                const agentId = file.replace(".jsonl", "").replace("agent-", "");
                subagentEvents.set(agentId, parseJsonlFile(join(subagentDir, file)));
            }
                ...
                const agentId = file
                    ...
                    .replace("agent-", "");
                try {
                    const meta = JSON.parse(readFileSync(join(subagentDir, file), "utf-8"));
