...
import { buildLifecycleRecords, processNewEvents, createInitialState, } from "./lifecycle-builder.js";
// ── Factory helpers ──────────────────────────────────────────────────
...
function makeAssistantEvent(overrides) {
    const { content, usage, model, stop_reason, ...rest } = overrides ?? {};
    return {
        type: "assistant",
        uuid: "uuid-" + Math.random().toString(36).slice(2),
...
describe("buildLifecycleRecords", () => {
    ...
    it("assistant event with tool_use content extracts toolName", () => {
        ...
        const result = buildLifecycleRecords("s1", [u1, a1], emptyMeta);
        const assistantEvent = result.lifecycleEvents.find((e) => e.eventUuid === "a1");
    ...
    it("user event with tool_result + is_error sets toolResultError", () => {
        ...
        const errorEvent = result.lifecycleEvents.find((e) => e.eventUuid === "u2");
    ...
    it("events before first turn boundary get turnNumber 0", () => {
        ...
        const preTurnEvent = result.lifecycleEvents.find((e) => e.eventUuid === "a0");
        expect(preTurnEvent.turnNumber).toBe(0);
        // The actual turn is turn 1
        const turnEvent = result.lifecycleEvents.find((e) => e.eventUuid === "a1");
...
describe("processNewEvents", () => {
    it("with null state initializes correctly", () => {
        const u1 = makeUserEvent({ timestamp: "2026-03-28T10:00:00Z" });
        const a1 = makeAssistantEvent({ timestamp: "2026-03-28T10:01:00Z" });
        const { records, state } = processNewEvents("s1", [u1, a1], emptyMeta, null);
    ...
    it("mid-turn extends current turn without new boundary", () => {
        ...
        const { records, state: state2 } = processNewEvents("s1", [a2], emptyMeta, state1);
    ...
    it("new boundary finalizes previous turn", () => {
        ...
        const { records, state: state2 } = processNewEvents("s1", [u2, a2], emptyMeta, state1);
...
//# sourceMappingURL=lifecycle-builder.test.js.map