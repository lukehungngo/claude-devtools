...
function isTurnBoundary(event) {
    ...
    if (userEvent.userType !== "external")
        return false;
    ...
    if (!Array.isArray(content))
        return false;
    return content.some((item) => item.type === "text" && "text" in item && (item.text ?? "").trim().length > 0);
}
...
function deriveTurnStatus(lastEvent) {
    if (lastEvent.type === "assistant") {
        const asst = lastEvent;
...
function extractToolResultError(event) {
    const content = event.message?.content;
    if (!Array.isArray(content))
        return false;
    for (const item of content) {
...
function processEvent(ctx, event) {
    ...
    if (event.type === "user") {
        toolResultError = extractToolResultError(event);
    }
...
export function buildLifecycleRecords(sessionId, events, subagentMeta) {
    if (events.length === 0) {
...
export function processNewEvents(sessionId, newEvents, subagentMeta, state) {
    const currentState = state ?? createInitialState();
    // Rebuild agent lifecycle map from state's knownAgents
    const agentLifecycleMap = new Map();
    ...
    // Build updated state
    const updatedKnownAgents = new Map();
...
//# sourceMappingURL=lifecycle-builder.js.map