...
describe("POST /open-file", () => {
    let app;
    const mockSpawnSync = child_process.spawnSync;
    const mockExecSync = child_process.execSync;
    beforeEach(() => {
        app = express();
        app.use(setupRoutes());
        vi.clearAllMocks();
    ...
    it("uses spawnSync (not execSync) for VS Code --goto", async () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith("code", ["--goto", "/tmp/test.ts:42"], { timeout: 5000 });
    ...
    it("uses spawnSync (not execSync) for EDITOR fallback", async () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith("nano", ["/tmp/test.ts"], { timeout: 5000 });
        expect(mockExecSync).not.toHaveBeenCalled();
    });
    it("falls back to vim when EDITOR is not set", async () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith("vim", ["/tmp/test.ts"], { timeout: 5000 });
        expect(mockExecSync).not.toHaveBeenCalled();
    });
...
describe("GET /sessions/:projectHash/:sessionId/git-diff", () => {
    let app;
    const mockSpawnSync = child_process.spawnSync;
    const mockDiscoverSessions = discoverSessions;
    beforeEach(() => {
        app = express();
        app.use(setupRoutes());
        vi.clearAllMocks();
    ...
    it("returns git diff stat and full diff for a valid session", async () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith("git", ["diff", "--stat", "--no-color"], expect.objectContaining({ cwd: "/tmp/myrepo" }));
    });
...
describe("GET /sessions/:sessionId/permissions-info", () => {
    let app;
    const mockGetAllowances = getSessionAllowances;
    const mockGetPending = getPendingPermissions;
    const mockDiscoverSessions = discoverSessions;
    beforeEach(() => {
        app = express();
        app.use(setupRoutes());
