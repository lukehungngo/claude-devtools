...
const mockWriteFileSync = writeFileSync;
...
describe("PUT /sessions/:projectHash/:sessionId/memory", () => {
    ...
    it("writes CLAUDE.md content successfully", async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith("/tmp/project/CLAUDE.md", "# Updated Content", "utf-8");
    });
    it("rejects path traversal attempts in projectHash", async () => {
        ...
        if (res.status === 200) {
            expect(mockWriteFile).toHaveBeenCalledWith("/tmp/project/CLAUDE.md", "# Hack", "utf-8");
        }
    });
});
...
describe("POST /sessions/:sessionId/init", () => {
    ...
    it("creates CLAUDE.md scaffold when it does not exist", async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith("/tmp/project/CLAUDE.md", expect.stringContaining("# Project Name"), "utf-8");
        expect(mockWriteFile).toHaveBeenCalledWith("/tmp/project/CLAUDE.md", expect.stringContaining("## Build & Test"), "utf-8");
    });
});
//# sourceMappingURL=routes-settings.test.js.map