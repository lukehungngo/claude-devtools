...
describe("POST /open-file — shell metacharacter sanitization (P2b)", () => {
    ...
    const routesSource = fs.readFileSync(path.resolve(import.meta.dirname, "routes", "session-routes.ts"), "utf-8");
    it("should validate EDITOR env var against shell metacharacters before use in execSync", () => {
        ...
        expect(hasEditorTest, "routes.ts must validate the editor variable with a regex .test(editor) call before passing to execSync").toBe(true);
    });
    it("should use execFileSync instead of execSync for open-file, OR sanitize filePath before shell interpolation", () => {
        ...
        const hasSafeCharRegex = /\[.*\^.*a-zA-Z0-9.*_.*\\?-.*\..*\/?\]/.test(routesSource);
        expect(hasSafeCharRegex, "routes.ts must either use execFileSync or validate filePath with a safe-char regex before calling execSync").toBe(true);
    });
});
//# sourceMappingURL=routes-open-file.test.js.map