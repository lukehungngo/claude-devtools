...
describe("GET /api/sessions/:projectHash/:sessionId/files", () => {
    ...
    it("returns 404 when session is not found", async () => {
        const app = buildApp();
        const res = await request(app).get("/api/sessions/no-hash/no-session/files?prefix=");
    ...
    it("returns empty files array when cwd does not exist", async () => {
        ...
        const app = buildApp();
        const res = await request(app).get("/api/sessions/hash-1/sess-1/files?prefix=");
    ...
    it("returns file list for valid session and empty prefix", async () => {
        ...
        const res = await request(app).get("/api/sessions/hash-1/sess-1/files?prefix=");
        expect(res.status).toBe(200);
        expect(res.body.files).toEqual(expect.arrayContaining(["src/", "package.json", "README.md"]));
    });
    ...
    it("limits results to 20 entries", async () => {
        ...
        vi.mocked(fs.readdirSync).mockImplementation(() => manyEntries);
        const app = buildApp();
        const res = await request(app).get("/api/sessions/hash-1/sess-1/files?prefix=");
        expect(res.status).toBe(200);
    ...
    it("filters by prefix for nested paths", async () => {
        ...
        const app = buildApp();
        const res = await request(app).get("/api/sessions/hash-1/sess-1/files?prefix=src/comp");
        expect(res.status).toBe(200);
...
//# sourceMappingURL=routes-files.test.js.map