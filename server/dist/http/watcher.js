 ...
 */
export function buildNewEventsMessage(filePath, events) {
    return {
        type: "new-events",
        filePath,
 ...
 */
export function buildNewSessionMessage(filePath) {
    return {
        type: "new-session",
        filePath,
        sessionId: extractSessionIdFromPath(filePath),
...
function storeLifecycleData(state, filePath, events) {
    const debugDb = state.debugDb;
    if (!debugDb || events.length === 0)
        return;
    try {
        const sessionId = extractSessionIdFromPath(filePath);
        const builderState = builderStates.get(sessionId) ?? null;
        const { records, state: newState } = processNewEvents(sessionId, events, new Map(), // subagentMeta — not available in watcher context
        builderState);
        ...
        debugDb.insertEventBatch(records.lifecycleEvents.map((e) => ({
            ...e,
