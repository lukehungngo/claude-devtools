...
import { addPermissionRequest, resolvePermissionRequest, getPendingPermissions, getPermissionStatus, addSessionAllowance, getSessionAllowances, } from "../../hooks/permission-handler.js";
...
export function createSessionRoutes({ state }) {
    const router = Router();
    // List all sessions
    ...
    router.get("/sessions/:sessionId/permissions-info", (req, res) => {
        try {
            ...
            const pendingCount = getPendingPermissions().filter((p) => p.sessionId === sessionId).length;
    ...
    router.get("/sessions/:projectHash/:sessionId", (req, res) => {
        try {
            ...
            const sessions = discoverSessions();
            const session = sessions.find((s) => s.projectHash === projectHash && s.id === sessionId);
            if (!session) {
            ...
            if (!cacheHit) {
                const { mainEvents, subagentEvents, subagentMeta: loadedMeta } = loadFullSession(session);
                subagentMeta = loadedMeta;
                metrics = computeMetrics(session, mainEvents, subagentEvents, subagentMeta);
                // Merge main + subagent events, sorted by timestamp
                const allSubEvents = [];
                for (const evts of subagentEvents.values()) {
                    allSubEvents.push(...evts);
                }
                allEvents = [...mainEvents, ...allSubEvents].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
                // Store in cache if stat is available
                try {
                    const stat = statSync(session.path);
                    metricsCache.set({ filePath: session.path, size: stat.size, mtimeMs: stat.mtimeMs }, { metrics, events: allEvents, subagentMeta });
                }
                catch {
            ...
            if (state?.debugDb) {
                try {
                    const sortedEvents = [...allEvents].sort((a, b) => (a.timestamp ?? "").localeCompare(b.timestamp ?? ""));
                    const records = buildLifecycleRecords(session.id, sortedEvents, subagentMeta);
                    ...
                    state.debugDb.insertEventBatch(records.lifecycleEvents.map((e) => ({
                        ...e,
        ...
        try {
            ...
            for (const entry of entries) {
                if (IGNORED_DIRS.has(entry.name))
                    continue;
                if (filterLower && !entry.name.toLowerCase().startsWith(filterLower))
                    continue;
    ...
    router.get("/sessions/:projectHash/:sessionId/events/:agentId", (req, res) => {
        try {
        ...
        try {
            ...
            for await (const message of sessionManager.sendMessage(sessionId, prompt, parsedImages)) {
                ...
                if (state && (msg.type === "assistant" || msg.type === "user") && msg.uuid) {
                    broadcast(state, buildNewEventsMessage(`live://${sessionId}`, [msg]));
                }
            }
    ...
    router.post("/sessions/:sessionId/effort", (req, res) => {
        ...
        const success = sessionManager.setEffortLevel(req.params.sessionId, level);
        if (!success) {
            return res.status(404).json({ error: "Session not found" });
        }
    ...
    router.post("/sessions/:sessionId/rewind", async (req, res) => {
        ...
        try {
            const result = await sessionManager.rewindFiles(req.params.sessionId, userMessageId, dryRun === true);
            res.json(result);
        }
        catch (err) {
...
`;
    ...
    const sessionAdditionalDirs = new Map();
