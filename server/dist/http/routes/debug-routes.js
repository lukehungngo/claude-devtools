...
export function createDebugRoutes({ state }) {
    const router = Router();
    ...
    router.get("/debug/sessions/:sessionId/turns/:turnNumber/agents", (req, res) => {
        ...
        try {
            const agents = state.debugDb.getAgentLifecycles(req.params.sessionId, parseInt(req.params.turnNumber));
            res.json(agents);
        }
        catch (err) {
    ...
    router.get("/debug/sessions/:sessionId/turns/:turnNumber/events", (req, res) => {
        ...
        try {
            ...
            const events = state.debugDb.getLifecycleEvents(req.params.sessionId, parseInt(req.params.turnNumber), agentId);
            res.json(events);
        }
        catch (err) {
    ...
    router.get("/debug/sessions/:sessionId/turns/:turnNumber/graph", (req, res) => {
        ...
        try {
            const upToEvent = parseInt(req.query.upToEvent) || 999999;
            const graph = state.debugDb.getGraphAtEvent(req.params.sessionId, parseInt(req.params.turnNumber), upToEvent);
            res.json(graph);
        }
        catch (err) {
